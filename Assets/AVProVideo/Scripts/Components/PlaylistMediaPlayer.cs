...
// 						Events.Invoke(mp, et, errorCode);
        ...
        public bool PrepareVideoFile(int index)
        {
            ...
            if (_playlist.HasItemAt(index))
            {
                ...
                _nextItem = _playlist.Items[index];

                if (_nextItem.filePath.Substring(_nextItem.filePath.Length - 3).ToUpper() == "MP4")
                {
                    NextPlayer.OpenVideoFromFile(NextPlayer.m_VideoLocation, _nextItem.filePath, false);
                }
                else
                    PrepareVideoFile(index + 1);
